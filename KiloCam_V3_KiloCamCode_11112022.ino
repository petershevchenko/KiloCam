...
unsigned long darkAlarmInterval = 120;  // Set Dark period sleep duration 3600 = 1 hr
...
unsigned int Dusk = 18; // Specify the latest hour (24 hour clock) you want captures to occur in Or the time to move to day action
  ...
  if (alarmIsrWasCalled)
  {
    ...
    { 
      if  (hour(t) >= Dawn && hour(t) <= Dusk) { //Only run camera for capture if current RTC time is between dawn and dusk
        ...
        // ********** Comment out the three lines below if you do not want dark action *********
...
// flash two short times to show unit is awake
    ...
    digitalWrite(13, HIGH);  // Flash the blue LED to show cycle about to start
    delay(500);
    digitalWrite(13, LOW);
    ...
    digitalWrite(13, HIGH);  // Flash the blue LED to show timestamp sent
    delay(500);
    digitalWrite(13, LOW);
